...
const WebcamComponent = () => {
  ...
  const calculateBodyMeasurements = (pose) => {
    ...
    if (
      ...
      const leftLegLength =
        calculateDistance(keypoints.leftHip, keypoints.leftKnee) +
        calculateDistance(keypoints.leftKnee, keypoints.leftAnkle);
      const rightLegLength =
        calculateDistance(keypoints.rightHip, keypoints.rightKnee) +
        calculateDistance(keypoints.rightKnee, keypoints.rightAnkle);
      const totalHeight =
        calculateDistance(keypoints.leftShoulder, keypoints.leftHip) +
        (leftLegLength + rightLegLength) / 2;

      setMeasurements({
        shoulderWidth: shoulderWidth.toFixed(2),
        hipWidth: hipWidth.toFixed(2),
        torsoHeight: torsoHeight.toFixed(2),
        leftLegLength: leftLegLength.toFixed(2),
        rightLegLength: rightLegLength.toFixed(2),
    ...
    <div style={{ position: "relative", width: "100%", height: "100vh" }}>
      ...
      {Object.keys(measurements).length > 0 && (
        ...
        >
          ...
          <p>Left Leg Length: {measurements.leftLegLength} px</p>
          <p>Right Leg Length: {measurements.rightLegLength} px</p>
          <p>Total Height: {measurements.totalHeight} px</p>
        </div>
      )}
    </div>
  );
